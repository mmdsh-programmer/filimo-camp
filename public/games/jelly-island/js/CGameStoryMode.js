function CGameStoryMode(oData, iLevel){
    var _bTouchActive;
    var _bBlock;
    var _bInitGame;
    var _bTimeSpawn;
...
//        }
        ...
        var oTween = createjs.Tween.get().call(s_oGame._fallFaces);
        oTween.paused = true;
    ...
    this._hintCheckMovesAvailable = function(){
        ...
        if(_aHintArray.length > 0){
            ...
            if(!bSpecialElement){
               ...
               var oShuffleText = new CFormatText(-300, CANVAS_HEIGHT/2, TEXT_SHUFFLE, PRIMARY_FONT_COLOR, s_oStage, "#ef8297", 50, "center");
    ...
    this._checkWin = function(){
        ...
        _bInitGame = false;
        _iTimeBonusScore = Math.round( (_iTimeElaps/1000) * 1 );
        this.gameOver(true);
    };
    ...
    this.onNextLevel = function(){
        ...
        if(s_iCurLevel === LEVEL_MATRIX.length){
    ...
    this.onExitHelp = function () {
        ...
        var szStrokeColor = STROKE_COLOR_STAGE[Math.round(Math.random())];
        
        var oStageText = new CFormatText(-300, CANVAS_HEIGHT/2, sprintf(TEXT_STAGE, s_iCurLevel), PRIMARY_FONT_COLOR, s_oStage, szStrokeColor, 90, "center");
    ...
    this.gameOver = function(bWin){  
        ...
        if(!bWin){
            ...
            var oStageClearSound  = playSound("stage_clear", 0.75, false);
            
            oStageClearSound.once("end",function(){
                fadeSound("soundtrack", 0, SOUNDTRACK_VOLUME_IN_GAME, 500);
            });
        }
    ...
    EXTRA_FACE_MULTIPLIER = oData.extra_item_multiplier;

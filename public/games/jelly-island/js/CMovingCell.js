function CMovingCell(iX, iY, iType, oParentContainer){
    ...
    this.fall = function(iX, iY, iTimeMulti){
        var iTotTime = TIME_FALL*iTimeMulti;
        createjs.Tween.get(_oFace).wait(iTotTime - 100).call(function(){s_oSoundMatching.fallPieces();});
        var oTween = createjs.Tween.get(_oFace).to({x:iX, y:iY}, iTotTime, createjs.Ease.backOut).call(s_oGame.onFinishFall, null, s_oGame);
        s_oGame.getTweensGroup().push(oTween);
    };

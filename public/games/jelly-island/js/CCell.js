function CCell(iX, iY, iRow, iCol, oParentContainer, iType, bBlock, bTrap){
    ...
    this._init = function(iX, iY, iRow, iCol, oParentContainer, iType, bBlock, bTrap){
        ...
        _oGlowFace = createSprite(s_oJellySpriteSheet, "type_7",CELL_SIZE/2,CELL_SIZE/2,CELL_SIZE,CELL_SIZE);
        _oGlowFace.gotoAndStop("type_7");
        
        if(_iType === TYPE_CHANGING){
            _oBgChanging.visible = true;
           _oFace = createSprite(s_oJellySpriteSheet, "changing",CELL_SIZE/2,CELL_SIZE/2,CELL_SIZE,CELL_SIZE);
           _oFace.gotoAndPlay("changing");
           _oCellContainer.addChild(_oFace);
        } else if(_iType === TYPE_STAR){            
           _oFace = createSprite(s_oJellySpriteSheet, _iType,CELL_SIZE/2,CELL_SIZE/2,CELL_SIZE,CELL_SIZE);
        ...
        if(!bTrap){
            ...
            _aListeners["mousedown"] = _oTarget.on("mousedown", this._onCellClick);
    ...
    this.unload = function(){
        if(_iType >= 0){
            ...
            if(!bTrap){
                _oTarget.off("mousedown", _aListeners["mousedown"]);
                _oTarget.removeAllEventListeners();
            }            
        }
    ...
    this._glowFace = function(){
        createjs.Tween.get(_oGlowFace, {loop:true}).to({alpha:0},1000).to({alpha:1},1000);
    };
    
    this.setNewSpritesheet = function(){
        _oFace.spriteSheet = s_oJellySpriteSheet;
    };
    
    this._onCellClick = function(e){
        if(_iType === TYPE_CHANGING){
            var iChangedType = _oFace.currentFrame;
            s_oGame.checkCellClicked(iRow, iCol, iChangedType, e);
        } else {
            s_oGame.checkCellClicked(iRow, iCol, _iType, e);
        }
    };
    
    this.disableSwipeAction = function(){
        _oTarget.off("pressmove", _aListeners["pressmove"]);
        _oTarget.off("pressup",   _aListeners["pressup"]);
    };
    
    this.enableSwipeAction = function(){
        _aListeners["pressmove"] = _oTarget.on("pressmove", s_oGame.onPressMoveCell, this);
        _aListeners["pressup"]   = _oTarget.on("pressup",   s_oGame.onPressUpCell, this, false, {row: iRow, col: iCol});
    };

function CGame(oData,iCurLevel,iScore){
    var _bUpdate = false;
    var _bVictory;
    var _bGameOver;
    var _bWaitTime;
    ...
    this._init = function(iCurLevel,iScore){ 
        _oBg = createBitmap(s_oSpriteLibrary.getSprite('bg_game'));
        s_oStage.addChild(_oBg);
        
	var oSprite = s_oSpriteLibrary.getSprite('balls');
	BALL_DIM = Math.floor(oSprite.width/NUM_BALL_COLORS);
        
        var oData = {   
                        images: [oSprite], 
                        // width, height & registration point of each sprite
                        frames: {width: BALL_DIM, height: oSprite.height, regX: 0, regY: 0}, 
	...
	_iScore = iScore;
        _iCurLevel = iCurLevel;		
        this._initLevel(oSprite);
        
        _oInterface = new CInterface(_iScore,_oBallSpriteSheet);
    ...
    this._initLevel = function(oSprite){
        ...
        BALL_DIM_HALF = BALL_DIM/2;
        var iStepY   = BALL_DIM-5;
        BOARD_OFFSET_X = ( CANVAS_WIDTH - (BALL_DIM*BOARD_COLS))/2;

        var iStepPairX = BALL_DIM/2;	
        ...
        _iLimitDx = BALL_DIM*BOARD_COLS + BOARD_OFFSET_X;
        ...
        for ( var r = 0; r < BOARD_ROWS; r++ ){
            ...
            for ( var c = 0; c < BOARD_COLS; c++ ){
                ...
                iOffCellaX = BOARD_OFFSET_X + (c * BALL_DIM);
        ...
        _oLaunchingBall = createSprite(_oBallSpriteSheet,"ball_0",0,0,BALL_DIM,BALL_DIM);
    ...
    this._loadLevel = function(){
        ...
        _bKickBall = false;
	szLevel = s_aLevelsData[_iCurLevel];
        ...
        for ( var r = BOARD_ROWS-1; r > -1; r-- ){
            for ( var c = 0; c < BOARD_COLS; c++ ){
                ...
                if ( pointInRect( _vCurPosLaunchingBall, r1 ) === true && (_aMatId[r][c] === 1) ){
                    ...
                    var av = new Array();
                    if (c>0) {
                            vTemp1.set( _aOffsetBall[r][c-1].x+BALL_DIM_HALF, _aOffsetBall[r][c-1].y+BALL_DIM_HALF + _iPosWall);
                            av.push( { v:vTemp1, r:r, c:(c-1) } );
                    }
                    if (c<BOARD_COLS-1) {		
                            vTemp2.set( _aOffsetBall[r][c+1].x+BALL_DIM_HALF,_aOffsetBall[r][c+1].y+BALL_DIM_HALF + _iPosWall);
                            av.push( { v:vTemp2, r:r, c:(c+1) } );
                    }
                    vTemp3.set( _aOffsetBall[r-1][c].x+BALL_DIM_HALF,_aOffsetBall[r-1][c].y+BALL_DIM_HALF +_iPosWall);
                    av.push( { v:vTemp3, r:(r-1), c:c } );


                    // EVEN ROW
                    if ( (r % 2)===0 && c>0){
                            vTemp4.set( _aOffsetBall[r-1][c-1].x+BALL_DIM_HALF,_aOffsetBall[r-1][c-1].y+BALL_DIM_HALF + _iPosWall);
                    ...
                    var iJointTollerance = 0;

                    for ( var i = 0; i < av.length; i++ ){
                        
                        //BALL DIAMETER IS REDUCED OF iJointTollerance PIXEL TO FACILITATE JOINTS
						//trace("dist2: "+distance2( _vCurPosLaunchingBall, av[i].v));
						//trace("dimball: "+(dimBall2-(iJointTollerance*iJointTollerance)));
						//trace("_aMatId["+ av[i].r +"]["+ av[i].c +"]: "+_aMatId[ av[i].r ][ av[i].c ]);
                        if ( (distance2( _vCurPosLaunchingBall, av[i].v) < (dimBall2-(iJointTollerance*iJointTollerance))) && (_aMatId[ av[i].r ][ av[i].c ] > 1) ){
                                this._attachBall(r,c);
                                return true;			
                        }
                    }
   ...
   this._updateWallPosition = function(){
        ...
        for ( var i = 0; i < BOARD_COLS; i++ ){
                var oWallBitmap = createBitmap(oSpriteWall);	

                oWallBitmap.x = i*BALL_DIM + BOARD_OFFSET_X;
    ...
    this._updatePhysics = function() {
        ...
        if ( ( _oLaunchingBall.x <= BOARD_OFFSET_X+BALL_DIM_HALF ) || ( _oLaunchingBall.x >= _iLimitDx-BALL_DIM_HALF-2 ) ){
            _iBankBonus++; 
            _vCurDirLaunchingBall = reflectVectorV2( _vCurDirLaunchingBall, _vNormalForBounce);
        }

        //IF BALL HAS BEEN HOOKED, GET READY FOR THE NEXT LAUNCH
        if ( this._checkIfBallHooked() || _oLaunchingBall.y < 0){
                _bLaunch = false;
